...
class ConstructionInterestFees:
    ...
    def get_line_items_sum(self) -> float:
        ...
        return (
            self.construction_loan_interest
            + self.origination_fee
            + self.credit_enhancement_fee
            + self.bond_premium
            + self.cost_of_issuance
            + self.title_recording
            + self.taxes
            + self.insurance
            + self.other_amount
        )
    ...
    def is_valid(self) -> Tuple[bool, str]:
        ...
        if abs(self.total - line_sum) > 0.01:  # Allow small floating point differences
            return (
                False,
                f"Total ({self.total:,.2f}) does not match sum of line items ({line_sum:,.2f})",
            )
        return True, ""
...
class ApplicationData:
    ...
    file_path: str = ""
    construction_interest_fees: ConstructionInterestFees = field(
        default_factory=ConstructionInterestFees
    )
    ...
    def get_financing_costs_pct_hard_costs(self) -> Optional[float]:
        ...
        if self.new_construction_total and self.new_construction_total > 0:
            return (
                self.get_combined_financing_costs() / self.new_construction_total
            ) * 100
        return None
    ...
    def validate(self):
        ...
        if self.new_construction_total is None:
            self.validation_warnings.append(
                "New Construction total not found (using fallback)"
            )
