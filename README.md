...
Sources & Uses Budget  tab:
please extract each line item amount, capture any “Other” label text, and also pull the section totals (then compute a combined total)
1.Construction Interest & Fees
Construction Loan Interest, Origination Fee, Credit Enhancement/Application Fee, Bond Premium, Cost of Issuance, Title & Recording, Taxes, Insurance, and Other (with its description, “Deputy Inspection”), along with Total Construction Interest & Fees.
2. Permanent Financing
 Loan Origination Fee, Credit Enhancement/Application Fee, Title & Recording, Taxes, Insurance, Other (Specify), and Total Permanent Financing Costs.
@@Total Construction Interest & Fees” should equal the sum of the construction line items, and the “Total Permanent Financing Costs” should equal the sum of the permanent line items. If totals don’t reconcile or fields are missing, flag the application clearly. 
. For each application, compute the combined financing costs (construction total + permanent total), then calculate financing costs per unit, financing costs per SF (which is under the application tab - see below), and financing costs as a percentage of hard costs. Which is under “NEW CONSTRUCTION” on the Sources and Uses Budget tab, please extract that total as well;