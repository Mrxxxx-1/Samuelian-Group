...
def main():
    ...
    if not skip_download:
        print("Step 1: Downloading Excel files (first 10 for testing)...")
        excel_files = download_excel_files(url, limit=10)  # Limit to 10 for testing
    ...
    if not excel_files:
        ...
        if len(excel_files) > 10:
            print(f"Found {len(excel_files)} files, limiting to first 10 for testing...")
    ...
    if valid_apps:
        ...
        per_unit = [app.get_financing_costs_per_unit() for app in valid_apps if app.get_financing_costs_per_unit()]
        per_sf = [app.get_financing_costs_per_sf() for app in valid_apps if app.get_financing_costs_per_sf()]
